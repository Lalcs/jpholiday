...
class EmperorsBirthday(registry.BaseHoliday):
    ...
    def _check_holiday(self, date):
        if date.year <= 1988 and date.month == 4 and date.day == 29:
            return True
        # 2019: 国民の祝日に関する法律(昭和23年法律第178号)の一部改正
        elif date.year <= 2018 and date.month == 12 and date.day == 23:
            return True
        # 2019: 国民の祝日に関する法律(昭和23年法律第178号)の一部改正
    ...
    def _check_holiday(self, date):
        ...
        for holiday in registry.RegistryHolder.get_registry():
            ...
            if holiday.is_holiday((date + datetime.timedelta(days=-1))):
                self._HOLIDAY_NAME = '{} {}'.format(holiday.is_holiday_name((date + datetime.timedelta(days=-1))),
                                                    '振替休日')
                return True
